...
///
/// States with a populated `match_spec` are policy input and are validated
/// unchanged. States without one are query output: the selected `name` or
/// `mac` field becomes the match, all other schema-defined read-only fields
/// are removed with warnings, and a non-empty `device_type` becomes
/// `match.type`. Both input and final prepared states are schema-validated.
